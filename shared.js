...
async function sarvamTTS(text) {
  ...
  // Attempt fetch with one silent retry on cold-start failure
  let res;
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (attempt === 2) throw new Error('Cannot reach voice server. Check internet connection.');
...
function bootShared(onReady) {
  // Inject version into header subtitle
  const subEl = document.querySelector('.hdr-sub');
  if (subEl) {
    subEl.innerHTML = subEl.innerHTML.replace(/v6\.\d/g, VERSION);
  }
  updateStarDisplay();
  openDB()
    .then(database => {
